...
Current evaluation evidence uses Benchmark IDs `BENCH-*`, Maturity V3 capability levels, and the
artifact paths below. Historical percentage-based maturity claims are not current evidence.

## Evaluation requirements

| ID | Requirement | Implementation evidence | Executed evidence | Status |
|---|---|---|---|---|
| BENCH-001 | Metrics derive from observations | `benchmarks/evidence.py`, retrieval runner | `reports/evaluation/evidence` | Implemented and executed |
| BENCH-002 | Paired retrieval ablation | A0-A4 executed retrieval paths | `reports/evaluation/full-evaluation.json` | Implemented and executed |
| BENCH-003 | Independent lexical/dense/hybrid baseline | `external_retrieval_baseline.py` | Unit fixture only; external datasets absent | Implemented, external run open |
| BENCH-004 | HTTP concurrency distribution | `http_load_benchmark.py` | 100-request S profile | Implemented and executed |
| BENCH-005 | Worker throughput and duplicate detection | `worker_queue_benchmark.py` | 100-job S profile | Implemented and executed |
| BENCH-006 | On-disk graph scale | graph performance on isolated SQLite file | 500-node/1,000-edge S profile | Implemented and executed |
| BENCH-007 | Browser desktop/mobile metrics | `browser-benchmark.mjs` | exploratory repeated routes | Implemented and executed |
| BENCH-008 | Instrumentation overhead | paired enabled/disabled runner | `instrumentation-overhead.json` | Implemented and executed |
| BENCH-009 | Evidence-based maturity | `maturity_v3.py` | `maturity-v3.json` | Implemented and executed |
| BENCH-010 | Human and field validity | thesis protocol | `external-evidence-register.md` | External evidence open |
