...
## Summary
BerryBrain is now a free, source-available, self-hosted app for non-commercial use. The security model is a single local owner account per instance, not SaaS multi-user administration. Keep first-party login, email 2FA/recovery, anti-abuse controls, privacy/legal pages, and an English public marketing site.
