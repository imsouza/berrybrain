import threading
import unittest
from http.server import BaseHTTPRequestHandler, ThreadingHTTPServer

from benchmarks.http_load_benchmark import run_http_load


class _StatusHandler(BaseHTTPRequestHandler):
    def do_GET(self) -> None:
        body = b'{"status":"ok"}'
        self.send_response(200)
        self.send_header("Content-Type", "application/json")
        self.send_header("Content-Length", str(len(body)))
        self.end_headers()
        self.wfile.write(body)

    def log_message(self, format: str, *args: object) -> None:
        return


class HttpLoadBenchmarkTest(unittest.TestCase):
    def test_measures_real_concurrent_requests(self) -> None:
        server = ThreadingHTTPServer(("127.0.0.1", 0), _StatusHandler)
        thread = threading.Thread(target=server.serve_forever, daemon=True)
        thread.start()
        try:
            metrics, observations = run_http_load(
                f"http://127.0.0.1:{server.server_port}",
                requests=20,
                concurrency=4,
            )
        finally:
            server.shutdown()
            server.server_close()
            thread.join(timeout=2)

        self.assertEqual(metrics.successes, 20)
        self.assertEqual(metrics.failures, 0)
        self.assertGreater(metrics.throughput_rps, 0)
        self.assertGreater(metrics.latency_p95_ms, 0)
        self.assertEqual(len(observations), 20)

    def test_rejects_invalid_workload(self) -> None:
        with self.assertRaises(ValueError):
            run_http_load("http://127.0.0.1", requests=0)


if __name__ == "__main__":
    unittest.main()
