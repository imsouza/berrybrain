...
class AttachmentProcessingTest(unittest.TestCase):
    ...
    def test_text_attachment_becomes_extraction_and_graph_source(self) -> None:
        ...
        note_node = (
            self.session.query(GraphNodeRecord)
            .filter(GraphNodeRecord.type == "note")
            .one()
        )
        ...
        edge = (
            ...
            .filter(GraphEdgeRecord.type == "attached_to")
            .one()
        )
        self.assertEqual(edge.source_node_id, attachment_node.id)
        self.assertEqual(edge.target_node_id, note_node.id)
