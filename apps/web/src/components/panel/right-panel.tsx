...
export function RightPanel() {
  ...
  const reprocessNote = async () => {
    ...
    if (!response.ok) {
      w.toast("Erro ao reprocessar nota.", "error");
      return;
    }
  ...
  return (
    <aside className="w-72 flex-shrink-0 flex flex-col border-l border-border/50 bg-panel overflow-y-auto" aria-label="Painel contextual">
      <div className="flex items-center justify-between px-4 py-3.5 border-b border-border/50">
        <h2 className="text-xs font-semibold tracking-tight">{w.active ? w.active.title.slice(0, 18) : "Atividade"}</h2>
          ...
          <>
            ...
            <Section title="Status">
              <div className="text-xs text-muted space-y-1">
                ...
                <Row k="Salvo" v={w.autosave === "saved" ? "Sim" : "Não"} />
              </div>
            </Section>
                ...
                {connections.slice(0, 4).map(c => {
                  ...
                  return (
                    <div key={c.id} className="rounded-xl bg-surface p-2">
                      ...
                      <p className="line-clamp-3 text-[10px] text-muted/70">{c.reason || "Conexão sem explicação."}</p>
                      {!!c.evidence?.length && <p className="mt-1 truncate text-[10px] text-muted/45">Evidência: {c.evidence.slice(0, 2).join(" · ")}</p>}
                      <div className="mt-1.5 flex items-center gap-1">
                        ...
                        {c.status !== "confirmed" && <button className="ml-auto text-[10px] text-accent" onClick={() => updateConnection(c.id, "confirm")}>confirmar</button>}
                        {c.status !== "ignored" && <button className="text-[10px] text-muted/60" onClick={() => updateConnection(c.id, "ignore")}>ignorar</button>}
                      </div>
                    </div>
                  );
                })}
                {connections.length === 0 && <p className="text-[11px] text-muted/50">Nenhuma conexão real encontrada para esta nota.</p>}
              </div>
            </Section>

            <Section title="Ações">
              <div className="flex flex-wrap gap-1.5">
                <button className="rounded-lg bg-surface px-2.5 py-1.5 text-[10px] text-muted hover:text-foreground hover:ring-1 hover:ring-border/50" onClick={reprocessNote}>Reprocessar</button>
                <button className="rounded-lg bg-surface px-2.5 py-1.5 text-[10px] text-muted hover:text-foreground hover:ring-1 hover:ring-border/50" onClick={expandGraph}>Expandir grafo</button>
                <button className="rounded-lg bg-surface px-2.5 py-1.5 text-[10px] text-amber-600 hover:text-amber-500 hover:ring-1 hover:ring-amber-300/50" onClick={() => w.setGraphOpen(true)}>Ver no grafo</button>
              </div>
            </Section>

            <Section title="Insights relacionados">
              <div className="space-y-2">
                {relatedInsights.map(i => (
                  <div key={i.id} className="rounded-xl bg-surface p-2">
                    ...
                    {i.suggested_action && <p className="mt-1 text-[10px] text-muted/55">Ação: {i.suggested_action}</p>}
                  </div>
                ))}
                {relatedInsights.length === 0 && <p className="text-[11px] text-muted/50">Nenhum insight relacionado salvo ainda.</p>}
              </div>
            </Section>
            ...
            <Section title="Atividade">
              ...
              {!w.jobs.some(j => j.payload?.note_path === w.active?.path) && <p className="text-[11px] text-muted/50">Nenhum job.</p>}
            </Section>
          </>
        ) : (
          <>
            <Section title="Processando agora">
              ...
              {!w.jobs.some(j => j.status === "running") && (
                <p className="text-[11px] text-muted/50">{w.jobs.filter(j => j.status === "pending").length ? `${w.jobs.filter(j => j.status === "pending").length} pendentes` : "Em dia"}</p>
              )}
            </Section>

            <Section title="Stats">
              <div className="grid grid-cols-2 gap-2">
                <Stat label="Notas" val={w.stats?.notes ?? w.notes.length} />
                <Stat label="Conexões" val={w.stats?.connections ?? 0} />
            ...
            <Section title="Jobs recentes">
              ...
              {w.jobs.length === 0 && <p className="text-[11px] text-muted/50">Nenhum job.</p>}
            </Section>
          </>
        )}
      </div>
