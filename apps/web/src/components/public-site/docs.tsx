...
In the hosted browser-storage build, a compatible cloud provider is mandatory. NVIDIA NIM,
OpenAI, DeepSeek, Groq, OpenRouter, and custom public HTTPS OpenAI-compatible endpoints are
supported. The key stays in that browser's IndexedDB, is excluded from exports, and is sent through
...
In the hosted web app, cloud processing is required but NVIDIA NIM is not. Select a preset or a
custom public HTTPS OpenAI-compatible endpoint. Local Ollama fields are hidden because a Netlify
browser deployment cannot reach the user's local Worker container reliably.
