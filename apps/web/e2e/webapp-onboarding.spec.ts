...
test.describe("Hosted web app onboarding", () => {
  ...
  test("allows skipping the tour but requires verified NVIDIA NIM setup", async ({ page }) => {
    await page.route("**/api/browser-ai/models", (route) => route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({ connected: true, provider: "nvidia-nim", models: ["qwen/test-model"] }),
    }));
    ...
    await page.getByLabel("NVIDIA NIM API Key").fill("nvapi-test-key-that-is-long-enough");
    ...
    await expect(page.getByRole("heading", { name: "Connect NVIDIA NIM to continue." })).toHaveCount(0);
    ...
    await expect(page.getByText("NVIDIA NIM cloud API (required)", { exact: true })).toBeVisible();
